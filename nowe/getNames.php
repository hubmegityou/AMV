...
$sql = "SELECT $db_connect_passenger_id FROM $db_connect_tab WHERE $db_connect_trip_id =2";
...
foreach ($data as $object){ 
			$sql= "SELECT $db_passengers_firstname, $db_passengers_lastname FROM $db_passengers_tab WHERE 
				$db_passengers_id= $object[$db_connect_passenger_id]";
			$stmt = $connection->prepare($sql);    
			$stmt->execute();
			$dataSet = $stmt->get_result();
